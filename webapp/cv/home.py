...
import threading
import shutil
from dbHandler import retrieveData
from facerec import *
...
UPLOAD_FOLDER = "./face_samples/temp_criminal"
...
def register(entries, required, menu_var):
    ...
    for i, img in enumerate(img_list):
        ...
        id = registerCriminal(img, UPLOAD_FOLDER, i + 1)
        if(id != None):
            no_face.append(id)
    ...
    if(len(no_face) > 0):
        ...
        if(rowId >= 0):
            ...
            shutil.move(UPLOAD_FOLDER, os.path.join('face_samples', entry_data["Name"]))
